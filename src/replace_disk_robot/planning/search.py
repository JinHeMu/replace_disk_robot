...
tree expansion in joint space.  No MuJoCo/Pinocchio/FCL dependency is imported
here; the concrete collision behaviour is supplied by the caller.
"""
...
class RRTConnectPlanner:
    ...
    def _build_path(
        ...
        q_all = forward + backward[1:]  # avoid duplicating the connection node

        result: list[TrajectoryPoint] = []
        for i, q in enumerate(q_all):
            result.append(
                TrajectoryPoint(
                    time_from_start_s=i * self.path_time_step_s,
                    position_rad=np.asarray(q, dtype=float),
                )
            )
        return result
